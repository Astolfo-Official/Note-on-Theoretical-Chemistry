...
% 2 FFT 算法的原理
% 3 pade 逼近
\subsection{Borwein 积分}
\href{https://en.wikipedia.org/wiki/Borwein_integral}{Borwein 积分}是一个著名的不完全归纳失败的例子。他的形式为如下通项数列：
\begin{equation}\label{eq:borwein}
I_n=\int_0^{+\infty}\prod_{k=0}^{n}\frac{\sin\left(x/(2k+1)\right)}{x/(2k+1)}\dd{x}, \quad n=0,1,2,\cdots
\end{equation}
前六个积分值都等于$\pi/2$，但第七个积分值却小于$\pi/2$。它还有另外一个更精确但依然不完全归纳失败的变形：
\[J_n=\int_0^{+\infty}2\cos(x)\prod_{k=0}^{n}\frac{\sin\left(x/(2k+1)\right)}{x/(2k+1)}\dd x\]
他的前55个积分值都等于$\pi/2$，但第56个积分值却小于$\pi/2$。这两个积分的现象都非常令人惊讶，但是如果利用傅里叶变换来分析就一目了然。我们首先定义在$\mathbb{R}/\{0\}$上定义带参数$a$的 sinc 函数，记为$f_a(x)$：
\[f_a(x)=\frac{\sin(x/a)}{x/a}, \quad \int_{-\infty}^{+\infty}f_a(x)\dd{x}=a\pi\]
可以在$x=0$处补充定义$f_a(0)=1$使得函数在$\mathbb{R}$上连续，但是这不影响我们这里的讨论。$f_a(x)$函数的傅里叶变换是一个带参数$a$对矩形函数，记为$g_a(\omega)$：
\[g_a(\omega)=\left\{
\begin{array}{ll}
a\pi & \text{, if } |\omega|<1/a\\ 
a\pi/2 & \text{, if } |\omega|=1/a\\ 
0 & \text{, if } |\omega|>1/a
\end{array}\right.\]
上式中定义端点只是为了符合狄利克雷条件，对积分运算无影响。容易验证：
\[\frac{1}{2\pi}\int_{-\infty}^{+\infty}g_a(\omega)e^{i\omega x}\dd{\omega}=\frac{a}{2}\int_{-1/a}^{1/a}e^{i\omega x}\dd{\omega}=\eval{\frac{a}{2}\cdot\frac{1}{ix}e^{i\omega x}}_{-1/a}^{1/a}=\frac{a}{2}\frac{e^{ix/a}-e^{-ix/a}}{ix}=\frac{a}{2}\frac{2i\sin(x/a)}{ix}=\frac{\sin(x/a)}{x/a}\]
由对称性，将 Borwein 积分 [Eq.\eqref{eq:borwein}] 写成成$\mathbb{R}$上对积分，且记被积函数为$F_n(x)$：
\[I_n=\frac{1}{2}\int_{-\infty}^{\infty}F_n(x)\dd{x}=\frac{1}{2}\int_{-\infty}^{+\infty}\prod_{k=0}^{n}\frac{\sin\left(x/(2k+1)\right)}{x/(2k+1)}\dd{x}\]
由傅立叶变换的性质可知\textcolor{blue}{\textbf{上述无穷积分实际上就是$F_n(x)$的傅里叶变换在原点处的值$\mathscr{F}[F_n](0)$}}。利用乘积定理，$F_n(x)$是一系列 sinc 函数的乘积，其傅里叶变换就是一系列矩形函数的卷积：
\[\int_{-\infty}^{+\infty}\prod_{k=0}^{n}\frac{\sin\left(x/(2k+1)\right)}{x/(2k+1)}\dd{x}=\frac{1}{(2\pi)^n}[g_1 \ast g_{3} \ast \cdots \ast g_{2n+1}](0)\]
记$G_n(\omega)=(2\pi)^{-n}[g_1 \ast g_{3} \ast \cdots \ast g_{2n+1}](\omega)$，现在问题变成了递推计算$G_n(\omega)$在原点处的函数值。从第一项开始：
\begin{equation}\label{eq:first_term}
G_1(\omega)=\frac{1}{2\pi}(g_1\ast g_3)(\omega)=\frac{1}{2\pi}\int_{-\infty}^{+\infty}g_1(\xi)g_3(\omega-\xi)\dd{\xi}=\frac{1}{2}\int_{-1}^{1}g_3(\omega-\xi)\dd{\xi}=\frac{1}{2}\int_{\omega-1/3}^{\omega+1/3}g_3(\xi)\dd{\xi}
\end{equation}
由$g_3$的定义式可知，只需要考虑变量$|\omega-\xi|<1/3$的情况，也即考虑区间$[\omega-1/3,\omega+1/3]$和$[-1,1]$的交集。
\begin{itemize}
\item 当$|\omega|\leqslant 2/3$时，区间$[\omega-1/3,\omega+1/3]$完全落在$[-1,1]$内，因此：
\[G_1(\omega)=\frac{1}{2}\cdot\frac{2}{3}\cdot3\pi=\pi\]
\item 当$2/3<|\omega|<4/3$时，两区间的重叠长度变为$4/3-|\omega|$，因此：
\[G_1(\omega)=\frac{1}{2}\cdot\left(\frac{4}{3}-|\omega|\right)\cdot3\pi=2\pi-\frac{3\pi}{2}|\omega|\]
\item 当$|\omega|\geqslant 4/3$时，两区间无重叠，因此：
\[G_1(\omega)=0\]
\end{itemize}
上面的分类这说明了两个矩形窗卷积以后得到一个梯形函数，中间平台的半宽恰好由$1$变为$1-1/3$，我们只关心中间平台的部分。根据公式\eqref{eq:first_term} 中的最后一个积分可以发现递推关系：\textcolor{blue}{\textbf{当把第$k$项卷积的矩形窗$g_{2k+1}$加入前$k-1$项的卷积时，平台半宽会减少$1/(2k+1)$，而平台中心$\omega=0$处的数值保持$G_k(0)=\pi$}}。因此：
\[I_n=\frac{1}{2}\int_{-\infty}^{+\infty}\prod_{k=0}^{n}\frac{\sin\left(x/(2k+1)\right)}{x/(2k+1)}\dd{x}=\frac{1}{2}\cdot\frac{1}{(2\pi)^n}[g_1 \ast g_{3} \ast \cdots \ast g_{2n+1}](0)=\frac{1}{2}G_n(0)=\frac{\pi}{2}\]
...
\[I_0=I_1=\cdots=I_6=\frac{\pi}{2}, \qquad I_7<\frac{\pi}{2}\]
...
可知$J_n$对应的第一个 sinc 因子由$f_1(x)$变成了$2f_{1/2}(x)$，因此在频域中初始矩形窗由$g_1$变成了$2g_{1/2}$。它的平台高度仍为$\pi$，但平台半宽从$1$增大为$2$。因此同样的判据变成：
\[\sum_{k=1}^{n}\frac{1}{2k+1}<2\]
直接计算得到：
\[\frac{1}{3}+\frac{1}{5}+\cdots+\frac{1}{111}<2,\quad \frac{1}{3}+\frac{1}{5}+\cdots+\frac{1}{113}>2\]
上述两个例子在哲学层面说明了一件事：\textcolor{blue}{\textbf{如果我们想要通过归纳法来证明一个结论，那么我们必须要对归纳的判据有一个清晰的理解，如数学归纳法，否则就可能会遇到不完全归纳失败的情况}}。