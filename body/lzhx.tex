\section{量子力学基本假设}\label{sec:qm_basic}
...
\begin{equation}\label{eq:operator_qm}
...
\[v_{\rm{g}}=\dv{\omega}{k}=\frac{1}{\hbar}\dv{E}{k}=\frac{\hbar k}{m}\]
\section{平动量子化——势箱}
\subsection{一维势箱}
...
\begin{center}
    \includegraphics[scale=0.6]{fig/lzhx/1d_box.png}
\end{center}
...
\begin{center}
    \includegraphics[scale=0.6]{fig/lzhx/1d_wf.png}
\end{center}
...
观察[式\eqref{comma}]我们发现，升降算符作用到本征态上具有以下效果（升降算符能把当前本征态升降到相邻本征态）：
...
\begin{center}
    \includegraphics[scale=0.8]{fig/lzhx/morse.png}
\end{center}
...
\begin{center}
    \includegraphics[scale=0.55]{fig/lzhx/angular_momentum.png}
\end{center}
...
\begin{center}
    \includegraphics[scale=0.65]{fig/lzhx/spin.png}
\end{center}
...
\begin{definition}[群同态]\label{def:g}
...
\newpage
\section{单核多电子体系}
\subsection{波恩-奥本海默(Born-Oppenheimer)近似}\label{sec:bo}
...
系统的哈密顿量$\hat{H}$可以写成核的动能项$\hat{T}_{\rm nuc}$，电子的动能项$\hat{T}_{\rm el}$，核-核之间的势能$\hat{V}_{\rm nuc-nuc}$，核-电子之间的势能$\hat{V}_{\rm nuc-el}$，电子电子之间的势能$\hat{V}_{\rm el-el}$，在原子单位制下可以写成：
\begin{equation}\label{eq:hamiltonian_bo}
\begin{aligned}
\hat{H}&=\hat{T}_{\rm nuc}+\hat{T}_{\rm el}+\hat{V}_{\rm nuc-nuc}+\hat{V}_{\rm nuc-el}+\hat{V}_{\rm el-el}\\
&=-\sum_{I=1}^M\frac{1}{2M_I}\nabla^2_{\mathbf{R},I}-\sum_{i=1}^N\frac{1}{2}\nabla^2_{\mathbf{r},i}+\sum_{I=1}^{M}\sum_{J>I}^{M}\frac{Z_{I}Z_{J}}{|\mathbf{R}_{I}-\mathbf{R}_{J}|}-\sum_{i=1}^{N}\sum_{I=1}^{M}\frac{Z_{I}}{|\mathbf{r}_{i}-\mathbf{R}_{I}|}+\sum_{i=1}^{N}\sum_{j>i}^{N}\frac{1}{|\mathbf{r}_{i}-\mathbf{r}_{j}|}\\
\end{aligned}
\end{equation}
这里$M_I$是核质量，$Z_I$是核电荷。我们定义电子哈密顿量部分为$\hat{H}_{\rm el}=\hat{T}_{\rm el}+\hat{V}_{\rm nuc-nuc}+\hat{V}_{\rm nuc-el}+\hat{V}_{\rm el-el}$。这里我们引入 BO 近似，BO 近似认为\textcolor{blue}{\textbf{电子相较于原子核运动很快，所以认为核坐标变化后电子总能在瞬间达到新的平衡}}。BO 近似还有另外一个名字，定核近似，但需要注意\textbf{这不意味着认为核不动}。在 BO 近似后，核坐标$\mathbf{R}$作为参数引入电子的波函数$\psi(\mathbf{r}_1,\cdots,\mathbf{r}_N;\mathbf{R}_1,\cdots,\mathbf{R}_M)$，我们将整体波函数$\Psi$拆分成电子波函数$\psi$和核波函数$\phi$：
\[\Psi(\mathbf{R}_1,\cdots,\mathbf{R}_M,\mathbf{r}_1,\cdots,\mathbf{r}_N)=\chi(\mathbf{R}_1,\cdots,\mathbf{R}_M)\psi(\mathbf{r}_1,\cdots,\mathbf{r}_N;\mathbf{R}_1,\cdots,\mathbf{R}_M)\]
这里$\chi(\mathbf{R}_1,\cdots,\mathbf{R}_M)$和$\psi(\mathbf{r}_1,\cdots,\mathbf{r}_N;\mathbf{R}_1,\cdots,\mathbf{R}_M)$分别表示所有核和所有电子的整体波函数。由于核坐标$\mathbf{R}$是参数，因此$\hat{V}_{\rm nuc-nuc}$退化为常数$V_{\rm nuc-nuc}(\mathbf{R})$。在 BO 近似下，$\nabla^2_{\mathbf{R},I}\Psi(\mathbf{R},\mathbf{r}) \approx 0, \ \nabla_{\mathbf{R},I}\Psi(\mathbf{R},\mathbf{r}) \approx 0$。下面我们用$\mathbf{R},\mathbf{r}$表示核坐标电子坐标全体，计算薛定谔方程，先考虑$\hat{T}_{\rm nuc}\Psi(\mathbf{R},\mathbf{r})$：
\[\begin{aligned}
\hat{T}_{\rm nuc}\Psi(\mathbf{R},\mathbf{r})&=-\sum_{I=1}^M\frac{1}{2M_I}\left[\nabla^2_{\mathbf{R},I}\psi(\mathbf{r};\mathbf{R}) \cdot \chi(\mathbf{R})+2 \nabla_{\mathbf{R},I}\psi(\mathbf{r};\mathbf{R}) \cdot \nabla_{\mathbf{R},I}\chi(\mathbf{R}) + \psi(\mathbf{r};\mathbf{R}) \cdot \nabla^2_{\mathbf{R},I}\chi(\mathbf{R})  \right]\\ 
&\approx -\psi(\mathbf{r};\mathbf{R})\sum_{I=1}^M\frac{1}{2M_I}\nabla^2_{\mathbf{R},I}\chi(\mathbf{R})
\end{aligned}\]
再考虑电子哈密顿部分$\hat{H}_{\rm el}\Psi(\mathbf{R},\mathbf{r})=\chi(\mathbf{R})\hat{H}_{\rm el}\psi(\mathbf{r};\mathbf{R})=\chi(\mathbf{R})E_{\rm el}(\mathbf{R})\psi(\mathbf{r};\mathbf{R})$，这里我们假定各位已经知道了解定态薛定谔方程是解特征值问题\href{https://github.com/Astolfo-Official/Note-on-Functional-Analysis/blob/main/Note%20for%20Functional%20Analysis.pdf}{泛函讲义的例题6.9}，解出的特征值与电子坐标$\mathbf{q}$无关，合并整个哈密顿量：
\[\hat{H}\Psi(\mathbf{R},\mathbf{r})=-\psi(\mathbf{r};\mathbf{R})\sum_{I=1}^M\frac{1}{2M_I}\nabla^2_{\mathbf{R},I}\chi(\mathbf{R})+\chi(\mathbf{R})E_{\rm el}(\mathbf{R})\psi(\mathbf{r};\mathbf{R})=E(\mathbf{R})\chi(\mathbf{R})\psi(\mathbf{r};\mathbf{R})\]
利用分离变量获得电子薛定谔方程[Eq.\eqref{eq:qm_bo1}]与核运动方程[Eq.\eqref{eq:qm_bo2}]：
\begin{subequations}\label{eq:qm_bo}
\begin{align}
\hat{H}_{\rm el}\psi(\mathbf{r};\mathbf{R})&=E_{\rm el}(\mathbf{R})\psi(\mathbf{r};\mathbf{R})\label{eq:qm_bo1}\\
-\sum_{I=1}^M\frac{1}{2M_I}\nabla^2_{\mathbf{R},I}\chi(\mathbf{R})&=[E(\mathbf{R})-E_{\rm el}(\mathbf{R})]\chi(\mathbf{R})\label{eq:qm_bo2}
\end{align}
\end{subequations}

通过反复求解不同核坐标微小变化下的电子薛定谔方程可以获得电子能量$E_{\rm el}(\mathbf{R})$关于核坐标$\mathbf{R}$函数，我们称该函数为势能面(PES)。由于这种将电子波函数重新计算为无穷小变化的核几何形状的函数的过程让人想起热力学中绝热定理的条件，\textcolor{blue}{\textbf{但需要注意，BO 近似和绝热近似是两个不同的近似，这两者的区别在后续内容[\ref{sec:jrjs}]中有详细介绍，所以严格意义上讲 BO 近似获得的势能面只是绝热势能面的近似。}}

电子结构所关心的是电子薛定谔方程，解出给定核构型下的电子分布，而动力学关心的是核薛定谔方程，认为给定核构型下的电子结构是已知的。一个有意思的事是：我们求解氢原子的波函数时用的是只含电子项的薛定谔方程，但是我们依然说这个是精确解，这是为什么呢？\textcolor{blue}{\textbf{因为波恩-奥本海默近似本质上是假定变量可分离，因此对实际上确实可分离的体系这就不是一种近似}}。BO近似是必要的，毕竟二体以上的系统我们没法精确求解。

\newpage
\subsection{平均场(Mean field)近似}
在BO近似后，我们开始接触多电子体系，需要注意的是现在我们其实还是不能直接处理这样的多电子体系，原因还是是二体以上的系统没法精确求解，所以我们需要考虑进一步近似把多电子体系近似成我们能处理的单电子体系。一个多电子多核系统其电子哈密顿量可以表示成（不考虑核的势能）：
\[\hat{H}_{\rm el}=-\sum_{i=1}^N\frac{1}{2}\nabla^2_{\mathbf{r},i}+-\sum_{i=1}^{N}\sum_{I=1}^{M}\frac{Z_{I}}{|\mathbf{r}_{i}-\mathbf{R}_{I}|}+\sum_{i=1}^{N}\sum_{j>i}^{N}\frac{1}{|\mathbf{r}_{i}-\mathbf{r}_{j}|}\]

在平均场近似下（这里是 Hartree 近似，可以阅读拓展章节\ref{sec:hartree}），把其他电子对某电子的双体算符$(\hat{V}_{ij})$平均成其他电子对该电子产生的势场$(\hat{V}_{i})$，此时体系哈密顿算符可以被拆分成单个粒子的哈密顿算符之和：
\[\hat{H}_{\rm el}\approx \sum_{i=1}^{N}\hat{h}_{i}, \quad \hat{h}_i=-\frac{1}{2}\nabla^2_{\mathbf{r},i}-\sum_{I=1}^{M}\frac{Z_I}{|\mathbf{r}_{i}-\mathbf{R}_{I}|}+\hat{V}^{\rm MF}_i, \quad \hat{V}^{\rm MF}_i=\sum_{\substack{j=1\\j\ne i}}^{N}\int\frac{|\psi_j(\mathbf{r}_j)|^2}{|\mathbf{r}_i-\mathbf{r}_j|}\dd{\mathbf{r}_j}\]
这里电子波函数也被近似成各个单电子波函数的乘积$\Psi(\mathbf{r}_1,\cdots,\mathbf{r}_N)\approx\prod_{i=1}^{N}\psi_i(\mathbf{r}_i)$（这里先对波函数这样的拆分方法正确性按下不表），此时系统的总薛定谔方程可以被拆分成一系列的单体薛定谔方程：
\[\hat{h}_i\psi_i(\mathbf{r}_i)=\left[-\frac{1}{2}\nabla^2_{\mathbf{r},i}-\sum_{I=1}^{M}\frac{Z_I}{|\mathbf{r}_{i}-\mathbf{R}_{I}|}+\hat{V}^{\rm MF}_i\right]\psi_i(\mathbf{r}_i)=\varepsilon_i(\mathbf{R}_1,\cdots,\mathbf{R}_M)\psi_i(\mathbf{r}_i)\]
但此时哈密顿包含了波函数，因为你需要用波函数来计算电子密度从而构建平均场，即你要解的方程是一个$\hat{H}_{\rm el}[\{\psi_i\}]\Psi=E_{\rm el}\Psi$形式的本征值问题，你当然不能直接对角化哈密顿，此时需要迭代来收敛最终得到波函数，这种方法称为自洽场(Self-Consistent Field, SCF)。

\subsection{粒子全同性与泡利不相容原理}\label{sec:particle_identity}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.6]{fig/lzhx/fermi_hole.png}
    \caption{\label{fig:fermi_hole}自旋平行（费米子系统）或者反平行（玻色子系统）的两个电子的空间分布，括号内对讨论见后续章节\ref{sec:f_and_b}。}
\end{figure}
...
\begin{itemize}
    \item 1. 同电子组态，S越大越稳定；
    \item 2. S相同，L越大越稳定；
    \item 3. L、S相同，若电子数小于半满，J越小越稳定；若电子数大于半满，J越大越稳定。
\end{itemize}
...
\begin{center}
    \includegraphics[scale=0.3]{fig/lzhx/h2_energy_level.jpg}
\end{center}

如果我们调整两个氢原子核的间距，可以绘制两个态的势能面：
\begin{center}
    \includegraphics[scale=0.21]{fig/lzhx/h2+_pes.jpg}
\end{center}
...
\begin{figure}[htbp]
    ...
    \centering
    \includegraphics[width=5cm]{fig/lzhx/h2+_rho+.png}
    \caption{$\rho_+$的分布图像}
    \end{minipage}
    \begin{minipage}[t]{0.48\textwidth}
    \centering
...
\section{极简（并非）群论}
\begin{definition}[群]
给定一个非空集合 $G$ 在其上定义一个二元运算 $\cdot: G \times G \to G$，如果其满足：
\begin{enumerate}
...
\end{enumerate}
则称集合 $G$ 与 运算 $\cdot$ 构成一个群 $(G, \cdot)$。若 $|G| < \infty$，则称 $G$ 为有限群，$|G|$ 称为群的阶，即集合中的元素个数。若运算还满足交换律，则称 $G$ 为交换群（阿贝尔群）。
\end{definition}
由分子对称性构成的点群一般都是有限群，同时每个对称元素在欧式空间中都能通过一个矩阵来描述。在之前的章节中我们介绍了群同态[定义\ref{def:g}]，这是一个保持结合律的映射，因此我们也可以在点群 $(G, \cdot)$ 和数域 $\mathbb{F}$ 上线性空间 $V$ 中的所有可逆线性映射（$V$ 与 $\mathbb{F}^n$，同构，$V$ 中的可逆线性映射一一对应于 $\mathbb{F}^n$ 中的可逆矩阵）构成的群 $GL(V)$ 之间构建一个群同态，称之为群的矩阵表示：
\begin{definition}[群的矩阵表示]
群的矩阵表示 $\rho: \ G \to GL(V)$ 满足 
\[\exists \, \rho(g),\rho(h) \in GL(V), \quad \rho(g h) = \rho(g)\rho(h),\quad \forall \, g,h \in G\]
\end{definition}
...
\begin{definition}[子表示与不可约表示]
设 $\rho_V: \ G \to GL(V)$ 是一个表示，$W \subseteq V$ 是一个子空间。若
\[\rho_V(g) W \subseteq W, \quad \forall \, g \in G,\]
则称 $\rho_W: \ G \to GL(W)$ 是一个 $\rho_V: \ G \to GL(V)$ 的\textbf{子表示}。若 $\rho_V$ 只有平凡表示（$\{0\}$ 和 $\rho_V$ 自身），则称 $\rho_V$ 为\textbf{不可约表示}；否则称为可约表示。
\end{definition}
对于有限群 $G$ 的分解，有 Maschke 定理
\begin{theorem}[Maschke 定理]
设 $G$ 是有限群，$V$ 是域 $\mathbb{F}$ 上的有限维向量空间，且 $\mathbb{F}$ 的特征不整除 $|G|$（当 $\mathbb{F}=\mathbb{C}$ 或 $\mathbb{R}$ 时，特征为0，自动满足条件）。若 $\rho: G \to GL(V)$ 是 $G$ 的表示，且 $W \subseteq V$ 是 $V$ 的 $G$-不变子空间（即 $\rho(g)W \subseteq W$ 对所有 $g \in G$ 成立），则存在 $G$-不变子空间 $U \subseteq V$ 使得：
\[V = W \oplus U\]
...
\begin{definition}[共轭关系]
设 $G$ 是一个群，对$\forall \, g, h \in G, \ \exists \, x \in G$ 满足 $h = x g x^{-1}$，则称 $g$ 与 $h$ \textbf{共轭}记作 $g \sim h$。
\end{definition}
共轭关系$\sim$是等价关系，满足\textbf{自反性}$(g \sim g)$，\textbf{对称性}$(g \sim h \ \Rightarrow \ h \sim g)$， \textbf{传递性}$(g \sim h, \ h \sim k \ \Rightarrow \ g \sim k)$。

\begin{definition}[共轭类]
群 $G$ 的一个\textbf{共轭类}是指由共轭关系 $\sim$ 确定的一个等价类。元素 $g \in G$ 所在的共轭类记作 $\mathcal{C}(g)$ 或 $[g]$，即
\[\mathcal{C}(g) = \{ h \in G \mid \exists \, x \in G \text{ 使得 } h = x g x^{-1} \}.\]
\end{definition}

\begin{definition}[中心化子]
元素 $g \in G$ 的\textbf{中心化子}定义为 $C_G(g) = \{ x \in G \mid x g = g x \}$，即与 $g$ 交换的所有元素构成的子群。
\end{definition}
...
一个函数 $f: G \to \mathbb{F}$ 在每个共轭类上取常数值，称为 $G$ 上的\textbf{类函数}，如果对于所有 $g, h \in G$，满足
\[f(h g h^{-1}) = f(g)\]
\end{definition}
\begin{definition}[类函数空间]
设 $\mathcal{C}(G)$ 为 $G$ 上所有类函数构成的向量空间。在 $\mathcal{C}(G)$ 上定义内积：
\[\bra{\phi}\ket{\psi} = \frac{1}{|G|} \sum_{g \in G} \phi(g)^*\psi(g)\]
\end{definition}
...
\begin{definition}[特征标]\label{def:x}
对于表示 $\rho: \ G \to GL(V)$，其\textbf{特征标} $\chi_\rho: G \to \mathbb{F}$ 定义为
\[\chi_\rho(g) = \Tr[\rho(g)]=\Tr[R(g)]\]
\end{definition}
特征标具有以下重要性质：
\begin{enumerate}
\item \textbf{类函数}：$\chi_\rho(h g h^{-1}) = \chi_\rho(g)$，即特征标在共轭类上为常数。
\item \textbf{加法性}：若 $\rho = \rho_1 \oplus \rho_2$，则 $\chi_\rho = \chi_{\rho_1} + \chi_{\rho_2}$。
\item \textbf{乘法性}：若 $\rho = \rho_1 \otimes \rho_2$ 是张量积表示，则 $\chi_\rho(g) = \chi_{\rho_1}(g) \chi_{\rho_2}(g)$。
\item \textbf{维数信息}：$\chi_\rho(e) = \dim V$，其中 $e$ 是单位元。
\end{enumerate}
特征标是对群表示映射后的矩阵的迹，\textcolor{blue}{\textbf{与不可约表示数目相同}}。由于特征标是类函数（性质1），因此所有不可约特征标构成 $\mathcal{C}(G)$是类函数空间，\textcolor{blue}{\textbf{且其个数与共轭类的个数相同，都是对应线性空间的维度}}。
\begin{theorem}[正交定理]
有限群 $G$ 的所有不可约特征标构成 $\mathcal{C}(G)$ 的一组标准正交基。即若 $\chi_i, \chi_j$ 是两个不可约特征标，则
\[\bra{\chi_i}\ket{\chi_j} = \delta_{ij}\] 
\end{theorem}

\begin{definition}[群代数]
有限群 $G$ 在数域 $\mathbb{F}$ 上的\textbf{群代数} $\mathbb{F}[G]$（$\mathbb{F}$-代数），其维数为 $|G|$，定义为形式线性组合的集合：
\[\mathbb{F}[G] = \left\{ \sum_{g \in G} a_g g \mid a_g \in \mathbb{F} \right\}\]
\end{definition}
显然群代数 $\mathbb{F}[G]$ 中的元素对加法，数乘和乘法封闭。这就是一个用有限群 $G$ 的元素作为基底的线性空间。

\begin{definition}[左(右)正则表示]
设 $G$ 是一个有限群，$\mathbb{F}[G]$ 是 $G$ 的群代数。\textbf{左(右)正则表示} $L(R): G \to \mathrm{GL}(\mathbb{F}[G])$ 定义为：
...
\end{definition}
显然其满足$L(gh) = L(g)L(h), \quad R(gh) = R(g)R(h), \quad L(e) = R(e) = \mathrm{id}$（单位元）。如果我们选择自然基 $\{e_g \mid g \in G\}$，其中 $e_g$ 对应于群元素 $g$。在这个基下正则表示就是置换矩阵（把一个基换到另外一个基）：
\[[L(g)]_{h,k} = \delta_{g h, k} = \delta_{h, g^{-1} k}, \quad [R(g)]_{h,k} = \delta_{h g^{-1}, k} = \delta_{h, k g}\]
即 $L(g)$ 把基向量 $e_h$ 映射到 $e_{gh}$，$R(g)$ 把基向量 $e_h$ 映射到 $e_{hg^{-1}}$，其特征标为$\chi_{L}(g) = \chi_{R}(g) = |G|\delta_{g,\rm{id}}$。只有当 $g$ 是单位元时矩阵对角线才非零。由 Maschke 定理：

\begin{theorem}[正则表示的分解]
设 $G$ 是有限群，有 $r$ 个互不等价的不可约复表示 $\rho_i$，其维数分别为 $d_i$。则左(右)正则表示可以分解为：
\[L\text{ or }R \cong \bigoplus_{i=1}^r d_i \rho_i\]
\end{theorem}
现在我们可以证明维数公式：
\begin{theorem}[维数公式]
设 $G$ 是有限群，$|G|$ 是 $G$ 的阶。设 $G$ 有 $r$ 个互不等价的不可约复表示，其维数分别为 $d_1, d_2, \dots, d_r$（即 $d_i = \chi_i(e)$，其中 $\chi_i$ 是第 $i$ 个不可约特征标，$e$ 是 $G$ 的单位元）。则有：
\[\sum_{i=1}^r d_i^2 = |G|\]
\end{theorem}
\begin{proof}
以左正则表示为例，选择 $g=e$，其特征标 $\chi_L(e)$ 可以分解为
\[|G| = \chi_L(e) = \sum_{i=1}^r d_i \chi_i(e) = \sum_{i=1}^r d_i^2\]
\end{proof}

设 $G$ 有 $r$ 个共轭类 $C_k$，其中 $C_1 = \{e\}$。设 $|C_k|$ 是共轭类 $C_k$ 的大小。根据特征标的定义[\ref{def:x}]以及附近的讨论可知，特征标表是一个 $r \times r$ 的复数矩阵，其第 $i$ 行对应第 $i$ 个不可约特征标 $\chi_i$，第 $k$ 列对应共轭类 $C_k$。
\begin{theorem}[特征标表的行（列）正交关系]
\[\sum_{k=1}^r \frac{|C_k|}{|G|} \chi_i(C_k)^*\chi_j(C_k) = \delta_{ij}, \quad \sum_{i=1}^r \chi_i(C_k)^*\chi_i(C_\ell) = \frac{|G|}{|C_k|} \delta_{k\ell}\]
\end{theorem}
由正交关系，给定一个表示的特征标 $\chi$，可以由以下投影公式计算出如何分解成不可约表示 $\chi$ 的直和：
\begin{equation}\label{eq:projection}
\chi \cong \bigoplus_{i=1}^r a_i \chi_i, \quad a_i=\sum_{k=1}^r \frac{|C_k|}{|G|} \chi_i(C_k)^*\chi(C_k) 
\end{equation}

\newpage
\subsection{H\"uckel 分子轨道}
H\"uckel 近似是$S_{ij}=\delta_{ij}$，每个原子自身带有能量$\alpha$，最近邻的原子之间有交换能$\beta$（这不是 tight-binding 吗）。以环丁烯\ch{C_4H_4}为例子，这里只涉及环平面（假装平面）上的四个$p$-轨道。解其久期行列式获得本征值
...
0 & \beta & \alpha-E & \beta \\
\beta & 0 &  \beta & \alpha-E \\
\end{vmatrix}=0 \quad \Rightarrow \quad (E-\alpha)^2[(E-\alpha)^2-4\beta^2]=0\]
本征值和对应的本征态为
\[\left\{\begin{array}{ll}
E=\alpha-2\beta, & (\ket{1}-\ket{2}+\ket{3}-\ket{4})/2\\  
E=\alpha, & (\ket{1}-\ket{2}-\ket{3}+\ket{4})/2, \quad (\ket{1}+\ket{2}-\ket{3}-\ket{4})/2\\  
E=\alpha+2\beta, & \ket{1}+\ket{2}+\ket{3}+\ket{4})/2\\  
\end{array}\right.\]
\begin{center}
\includegraphics[scale=0.3]{fig/lzhx/chi.png}
\end{center}
\begin{center}
\includegraphics[scale=0.2]{fig/lzhx/D4h.png}
\end{center}
按照流程，先画出环丁烯的四个$p$-轨道，然后标号，按照特征标表里的对称元素进行对称操作。操作后，每一个$p$-轨道如果保持原样，对应的特征标分量 $\chi(C_k)$为1，相位颠倒为-1，如果不对称为0（比如跑别的位置上了）。根据上述流程，总特征标 $\chi=(4,0,0,0,-2,0,0,-4,0,2)$。利用投影公式[式\ref{eq:projection}]，对每一个特征标嗯算可知 $\chi=A_{2u}\oplus E_g \oplus B_{1u}$。根据特征标标提供的对称函数，可以轻易看出每个本征态的对称性$\ket{\alpha+2\beta}: \ A_{2u}, \ \ket{\alpha_1}: \ E_g(xz), \ \ket{\alpha_2}: \ E_g(yz), \ \ket{\alpha-2\beta}: \ B_{1u}$。当然分解总特征标不一定要投影公式，你观察能力够强也可以（笑死
\begin{center}
\includegraphics[scale=0.3]{fig/lzhx/c4h4.png}
\end{center}

\newpage
\subsection{群表示直积：红外拉曼活性判断}
计算量子力学中的矩阵元（如期望值、跃迁矩阵元）时，我们通常会遇到如下形式的积分：
\[\bra{f}\hat{O}\ket{i} = \int \psi_f^*(\bm{r}) \hat{O} \psi_i(\bm{r}) \dd{\bm{r}}\]
通过对称性可以判断这个积分是不是非零的，而对于上述积分，其对称性可以由积分算子和被积函数合成（回忆一下函数奇偶性，除了被积函数的奇偶性外也需要要求积分区域是对称的，由于积分区域的对称，可以认为其在各种操作下都不变，不然没法讨论奇偶性）：
\begin{lemma}[积分算子的对称性]
积分算子在整个对称群下是不变的（属于恒等表示）:
\[\forall \, g \in G, \quad \int f(\bm{r}) \dd{\bm{r}} = g\int f(\bm{r}) \dd{\bm{r}} = \int f(g^{-1}\bm{r}) \dd{\bm{r}}\]
或者说，积分测度 $\dd{\bm{r}}$ 在对称操作下不变。
\end{lemma}
所以积分的对称性将取决于被积函数的对称性。
\begin{theorem}[积分正交性定理]
设 $G$ 是一个紧致群（或有限群），在空间 $\mathcal{M}$ 上作用，$\dd{\bm{r}}$ 是 $G$-不变测度。设函数 $F: \mathcal{M} \to \mathbb{F}$ 是在群 $G$ 作用下按照不可约表示 $\rho^{(k)}$ 变换的基函数 $\{F_j\}$ 张成空间中的非零函数，且基函数 $\{F_j\}$ 满足以下关系
\[\forall \, g \in G, \ i=1,2,\cdots,d_k, \quad (g \cdot F_i)(\bm{r}) = \sum_{j=1}^{d_k} D^{(k)}_{ij}(g) F_j(\bm{r})\]
其中 $D^{(k)}(g)$ 是表示 $\rho^{(k)}$ 的矩阵表示，$d_k$是不可约表示 $\rho^{(k)}$ 的维度。那么，
\[\int_{\mathcal{M}} F(\bm{r}) \dd{\bm{r}} \neq 0 \quad \Leftrightarrow \quad \rho^{(k)}=\text{id}\]
更一般地，如果 $F$ 属于可约表示，则积分非零当且仅当 $\rho^{(k)}$ 的不可约分解中包含恒等表示。
\end{theorem}
而被积函数的对称性由到达态、算符和出发态的对称性直积合成$\rho^{\rm{total}}=\rho^{(f)}\otimes\rho^{(O)}\otimes\rho^{(i)}$，做直和分解
\[\rho^{\rm{total}} = \rho^{(f)}\otimes\rho^{(O)}\otimes\rho^{(i)} = \bigoplus_k a_k \rho^{(k)}\]
进而可以利用特征标的来简化群表示的分解。\textcolor{blue}{\textbf{对于红外与拉曼，波函数作用的算符是偶极算符 $\hat{\bm{\mu}}$ 和极化率算符 $\hat{\alpha}$，分别对应的对称函数是 $x,y,z$ 和 $pq, \ (p,q=x,y,z)$（需要根据特征标标查对应的特征标）}}，出发态和到达态的对称性是对应振动态的简正模的对称性（先不管电子对称性）。最后介绍一下如何获得简正模的对称性。
\begin{figure}[htbp]
\centering
\begin{minipage}[t]{0.49\textwidth}
\centering
\includegraphics[width=0.9\textwidth]{fig/lzhx/h2o.png}
\end{minipage}
\begin{minipage}[t]{0.49\textwidth}
\centering
\includegraphics[width=0.9\textwidth]{fig/lzhx/C2v.png}
\end{minipage}
\end{figure}

以\ch{H_2O}为例，按照点群对称性把三个原子的九个自由度摆出来，两个氢原子一组，氧原子一组，用上一小节提到的方法获得总特征标 $\chi=3A_1 \oplus 3B_1 \oplus A_2 \oplus 2B_2$，去除掉平动 $x,y,z$ 和振动 $R_x,R_y,R_z$ 的自由度留下振动 $(3N-6/3N-5)$ 的自由度 $\chi_{\rm{vib}}=2A_1 \oplus B_1$，分别对应对称伸缩和弯曲以及非对称伸缩。\textcolor{blue}{\textbf{（至少一维不可约表示）只有奇数阶的振动态体现简正模的对称性，偶数阶都是全对称（考虑谐振子产生算符$\hat{a}^{\dagger}$作用$n$次）}}。
\begin{center}
\includegraphics[width=0.55\textwidth]{fig/lzhx/h2o_vib.png}
\end{center}
...
\begin{center}
    \includegraphics{fig/lzhx/mo_pes.png}
\end{center}
...
\begin{center}
    \includegraphics{fig/lzhx/mo_symmetry.png}
\end{center}
...
\newpage
\section{附录：氢原子薛定谔方程求解(级数解法)\label{hydrgen}}
\subsection{分离变量法处理氢原子薛定谔方程}
考虑氢原子的定态薛定谔方程：
\[\hat{H}\Psi(\mathbf{r})=(\hat{T}+\hat{V})\Psi(\mathbf{r})=\left(-\frac{\hbar^2}{2m_e}\nabla^2+V\right)\Psi(\mathbf{r})=\left(-\frac{\hbar^2}{2m_e}\nabla^2-\frac{e^2}{4\pi\varepsilon_0|\mathbf{r}|}\right)\Psi(\mathbf{r})=E\Psi(\mathbf{r})\]
利用分离变量法\ref{sec:flbl}，我们假定定态波函数可以表示成$\Psi(\mathbf{r})=R(r)\Theta(\theta)\varPhi(\varphi)$，带入原薛定谔方程方程：
\[-\frac{\hbar^2}{2m_e}\left[\frac{\Theta\varPhi}{r^2}\frac{\partial}{\partial{r}}(r^2\frac{\partial R}{\partial{r}})+\frac{R\varPhi}{r^2\sin\theta}\frac{\partial}{\partial{\theta}}(\sin\theta\frac{\partial \Theta}{\partial{\theta}})+\frac{R\Theta}{r^2\sin^2 \theta }\frac{\partial^2 \varPhi}{\partial{\phi^2}}\right]=(E-V)R\Theta\varPhi\]
在方程两边同时乘上因子$r^2\sin^2 \theta/R\Theta\varPhi$，并简单整理：
\begin{equation*}
\begin{aligned}
...
\[\frac{\sin^2 \theta}{R}\frac{\partial}{\partial{r}}(r^2\frac{\partial R}{\partial{r}})+\frac{\sin\theta}{\Theta}\frac{\partial}{\partial{\theta}}(\sin\theta\frac{\partial \Theta}{\partial{\theta}})+\frac{2m(E-V)}{\hbar^2}r^2\sin^2 \theta=m^2\]
...
同样的，假定上述方程左右两边均等于$\beta$，于是氢原子定态薛定谔方程可以拆分成以下三个常微分方程：
\begin{subequations}
\begin{align}
&\frac{\partial^2 \varPhi}{\partial{\phi^2}}+m^2\varPhi=0 \label{eq:1.17_a}\\
&\frac{1}{R}\frac{\partial}{\partial{r}}(r^2\frac{\partial R}{\partial{r}})+\frac{2m(E-V)}{\hbar^2}r^2=\beta \label{eq:1.17_b}\\
&\frac{m^2}{\sin^2 \theta}-\frac{1}{\Theta \sin\theta}\frac{\partial}{\partial{\theta}}(\sin\theta\frac{\partial \Theta}{\partial{\theta}})=\beta \label{eq:1.17_c}
\end{align}
\end{subequations}

\subsection{Φ方程的求解}
由球坐标中$\varphi$的范围及归一化条件容易求解方程 Eq.\eqref{eq:1.17_a}：
\[\varPhi(\phi)=\frac{1}{\sqrt{2\pi}}e^{im\phi}, \quad (m=0,\pm 1,\pm 2\cdot\cdot\cdot)\]

\begin{theorem}\label{theorem:czwy}
...
\subsection{Θ方程的求解}
对方程 Eq.\eqref{eq:1.17_c}，令$x=\cos\theta$，$y(x)=\Theta(\theta)$：
\[\frac{\mathrm{d} \Theta}{\mathrm{d} x}=\frac{\mathrm{d} \Theta}{\mathrm{d} \theta}\left ( \frac{\mathrm{d} x}{\mathrm{d} \theta} \right )^{-1}=-\frac{1}{\sin\theta} \frac{\mathrm{d} \Theta}{\mathrm{d} \theta}\]
则方程 Eq.\eqref{eq:1.17_c}可以改写成连带 Legendre 方程：
...
方程的存在唯一性由定理\ref{theorem:czwy}保证。

为了求解上述方程，我们需要从 Legendre 方程出发：
\begin{equation}\label{eq:Legendre}
\frac{\mathrm{d}}{\mathrm{d}{x}} \left [(1-x^2) \frac{\mathrm{d} y}{\mathrm{d}{x}} \right ]+\beta y=0
\end{equation}
其中自变量$x \in [-1,1]$，设方程 Eq.\eqref{eq:Legendre}的解在方程的常点$x_0=0$的邻域内可以唯一展开：
\[y=\sum_{k=0}^{+\infty}c_kx^k\]
将展开式带入 Legendre 方程，并整理：
\[\sum_{k=0}^{+\infty} \left \{ (k+2)(k+1)c_{k+2}-[k(k+1)-\beta]c_k \right \}x^k=0 \quad \Rightarrow \quad c_{k+2}=\frac{k(k+1)-\beta}{(k+2)(k+1)}c_k\]
按照奇偶性可以得到：
\[c_{2k}=\prod_{n=1}^{k}\frac{(2k-2)(2k-1)-\beta}{(2k)(2k-1)}c_0, \quad c_{2k+1}=\prod_{n=1}^{k}\frac{(2k)(2k-1)-\beta}{(2k)(2k+1)}c_1\]
将方程的解按奇偶项拆开：
\[y(x)=c_0y_0(x)+c_1y_1(x), \quad y_0(x)=1+\sum_{k=1}^{+\infty}\frac{c_{2k}}{c_0}x^{2k}, \quad y_1(x)=x+\sum_{k=1}^{+\infty}\frac{c_{2k+1}}{c_1}x^{2k+1}\]
...
此时级数$y(x)$退化成 $2l$ 项的有限多项式$P_l(x)$，我们得到了方程 Eq.\eqref{eq:Legendre}的解，也即 Legendre 多项式：
...
\begin{equation}\label{eq:1.17_d}
y=(x^2-1)^{\frac{m}{2}}v(x) \quad \Rightarrow \quad (1-x^2)v''-2(m+1)xv'+[\beta-m(m+1)]v=0
\end{equation}
对 Legendre 方程[Eq.\eqref{eq:Legendre}]求$m$次导：
...
发现其与方程 Eq.\eqref{eq:1.17_d}形式相同。于是，连带 Legendre 方程的解为：
...
至此，方程\ref{eq:1.17_c}的解可以写成：
\[\Theta_{l,m}(\theta)=(-1)^m\sin^m(\theta)P^m_l(\cos\theta)\]
为了满足归一化条件
\[\int_0^{\pi}\Theta_{l,m}^{*}(\theta)\Theta_{l^{'},m^{'}}(\theta)\sin\theta \dd\theta=\delta_{ll^{'}} \cdot \delta_{mm^{'}}\]
$\Theta(\theta)$最终可以表示成：
\[\Theta_{l,m}(\theta)=\sqrt{\frac{(2l-1)(l-|m|)!}{2(l+|m|)!}}P^{m}_l(\cos\theta), \quad |m| \leqslant l\]

此外，Legendre 多项式也可以通过 Rodrigue 公式定义：
\[P_l(x) \equiv \frac{1}{2^ll!} \left ( \frac{\mathrm{d}}{\mathrm{d}x} \right )^l(x^2-1)^l\]

\subsection{R方程的求解}
对方程 Eq.\eqref{eq:1.17_b}，我们要处理的是以下初值问题：
\[\frac{1}{R}\frac{\partial}{\partial{r}}(r^2\frac{\partial R}{\partial{r}})+\frac{2m(E-V)}{\hbar^2}r^2=\beta, \quad r>0, \quad V=-\frac{e^2}{4 \pi \varepsilon_0 r}\]
令$u \equiv rR$并整理可以得到：
\[-\frac{\hbar^2}{2m}\frac{\mathrm{d^2}u}{\mathrm{d}r^2}+ \left (\frac{\hbar^2}{2m}\frac{l(l+1)}{r^2}-\frac{e^2}{4 \pi \varepsilon_0r} \right )u=Eu\]
容易看出，当$r \rightarrow +\infty$时,
\[u=A\exp\left(-\frac{\sqrt{-2mE}}{\hbar}r\right)+B\exp\left(\frac{\sqrt{-2mE}}{\hbar}r\right)\]
当$E>0$时，容易看出$u(r)\nrightarrow0$，为非束缚态；当$E<0$时，取$B=0$使得当$r \rightarrow +\infty$时$u \rightarrow 0$。继续令：
...
\[j(j+1)c_{j+1}+2(l+1)(j+1)c_{j+1}-2jc_j+[\rho_0-2(l+1)]c_j=0 \quad \Leftrightarrow \quad c_{j+1}=\frac{2(l+1+j)-\rho_0}{(j+1)(j+2l+2)}c_j\]
...
因此$v(\rho)$近似为：
\[v(\rho) \sim c_0\sum_{j=0}^{+\infty}\frac{2^j}{j!}\rho^j=c_0e^{2\rho} \quad \Rightarrow \quad u(\rho) \sim c_0\rho^{l+1}e^{\rho}\]
...
这里的$n$也就是主量子数，从其定义式上我们可以很明显得看出角量子数的取值应为$l=0,1,2, \cdot\cdot\cdot ,n-1$。我们也能看出$\rho_0=2n$。再结合$\rho_0$与$\kappa$的定义式，我们可以得到能量表达式$E_n$：
\[E_n=- \left [\frac{m}{2\hbar^2} \left ( \frac{e^2}{4 \pi \varepsilon_0} \right )^2 \right ] \frac{1}{n^2}\]
和玻尔半径$a$：
\[\frac{4 \pi \varepsilon_0 \hbar^2}{me^2}=0.529 \times 10^{-10}\text{ m}\]
最终，再根据归一化条件
\[\int_0^{\infty}R_{n',l'}(r)^*R_{n,l}(r)r^2\dd{r}=\delta_{nn'}\delta_{ll'}\]
可以写出方程b的解$R_{n,l}(r)$：
...
因此，氢原子定态波函数的解$\varPsi_{n,l,m}(r,\theta,\phi)$为：
\[\varPsi_{n,l,m}(r,\theta,\phi)=\sqrt{ \left (\frac{2}{na} \right )^3 \frac{(n-l-1)!}{2n[(n+l)!]^3}}e^{-\frac{r}{na}}\left (\frac{2r}{na} \right )^l \left [L_{n-l-1}^{2l+1} \left ( \frac{2r}{na} \right ) \right ]Y_l^m(\theta,\phi)\]
