...
通过拉格朗日乘数法把正交归一条件引入泛函$\eta[\{\psi_i\},\lambda_{ij}]$进行变分，这里变分的目的是寻找到最佳的一组单电子轨道波函数$\{\psi_i\}$使得能量最小化，通过变分我们会获得一组单电子波函数$\{\psi_i\}$的约束方程。约定符号$\bra{\bm{r}}\ket{i}=\psi_i(\bm{r})\equiv\psi_i$，采用波函数表象时，正交归一条件写为：
\[\int \psi_i^*(\bm{r})\psi_j(\bm{r})\dd{\bm{r}}=\delta_{ij}\]
于是变分泛函为：
\begin{equation*}
\begin{aligned}
...
对双电子中的库伦项，有：
\[\frac{1}{2}\frac{\delta}{\delta \psi_p^*}\sum_{i,j}\iint\frac{\psi_i^*(\bm{r}_1)\psi_j^*(\bm{r}_2)\psi_i(\bm{r}_1)\psi_j(\bm{r}_2)}{r_{12}}\dd{\bm{r}_1}\dd{\bm{r}_2}=\sum_j\left(\int\frac{|\psi_j(\bm{r}')|^2}{|\bm{r}-\bm{r}'|}\dd{\bm{r}'}\right)\psi_p(\bm{r})\equiv\sum_j\hat{J}_j\psi_p(\bm{r})\]
同理，交换项的变分为：
\[-\frac{1}{2}\frac{\delta}{\delta \psi_p^*}\sum_{i,j}\iint\frac{\psi_i^*(\bm{r}_1)\psi_j^*(\bm{r}_2)\psi_j(\bm{r}_1)\psi_i(\bm{r}_2)}{r_{12}}\dd{\bm{r}_1}\dd{\bm{r}_2}=-\sum_j\left(\int\frac{\psi_j^*(\bm{r}')\psi_p(\bm{r}')}{|\bm{r}-\bm{r}'|}\dd{\bm{r}'}\right)\psi_j(\bm{r})\equiv-\sum_j\hat{K}_j\psi_p(\bm{r})\]
而正交归一约束项给出：
...
因此$\lambda_{ij}$构成厄密矩阵。由于占据轨道之间的酉变换不改变 Slater 行列式表示的能量，因此总可以在占据子空间内再做一次酉变换将其对角化，得到每个轨道对应的轨道能量$\varepsilon_i$，于是得到正则轨道下的 Hartree-Fock 方程和对应的平均场哈密顿量：
