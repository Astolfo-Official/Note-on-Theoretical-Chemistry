...
\[\left(\hat{T}_{\rm nuc}+E_m-E\right)\chi_m(\mathbf{R})\approx 0\]
如果只保留对角项$\hat{\Lambda}_{mm}$而忽略非对角项$\hat{\Lambda}_{mn}, \ (m \neq n)$，则得到比最简单 BO 近似更精细的绝热核方程。而真正导致电子态之间跃迁的，正是这些非对角的非绝热耦合项。

\subsection{非绝热耦合与绝热表象}
在绝热表象里，势能矩阵本身是对角的，所有电子态之间的混合都被塞进了核动能项。若把核波函数写成列向量$\bm{\chi}^{(a)}=(\chi_1^{(a)},\chi_2^{(a)},\cdots)^T$，并记一阶导数耦合矩阵元为
\[\left(\mathbf{T}_A^{(1)}\right)_{ji}=\bm{d}_{ji}^{A}\]
则耦合方程可以形式化地写成
\[\left[-\sum_A\frac{1}{2M_A}\left(\nabla_A+\mathbf{T}_A^{(1)}\right)^2+\mathbf{V}^{(a)}(\bm{R})-E\mathbbm{1}\right]\bm{\chi}^{(a)}=0\]
其中$\mathbf{V}^{(a)}_{ji}(\bm{R})=V_i(\bm{R})\delta_{ji}$。这个写法说明：在绝热表象中势能是对角的，但是核动能被“穿衣服”了，里面额外包含了非绝热耦合的信息。

导数耦合还可以写成更加直观的形式。对电子本征方程对核坐标求导，并在左边乘上$\bra{\phi_j^{(a)}}$，当$i\neq j$时有
\[\bm{d}_{ji}^{A}=\frac{\bra{\phi_j^{(a)}}(\nabla_A\hat{H}_{el})\ket{\phi_i^{(a)}}}{V_i(\bm{R})-V_j(\bm{R})}\]
这个式子直接告诉我们：当两张绝热势能面彼此接近时，分母会变小，导数耦合就会迅速变大。因此在避免交叉附近 BO 近似往往会失效，而在圆锥交叉附近导数耦合甚至会出现奇异行为。也就是说，\textcolor{blue}{\textbf{绝热表象最自然地适合描述远离简并区域的定态问题，但在势能面接近或相交时，核与电子自由度重新强烈纠缠起来，必须显式考虑非绝热跃迁}}。

\subsection{Diabatic 表象}
既然绝热表象中的麻烦主要来自导数耦合，一个自然的想法就是换一组电子基底，把这些一阶导数项尽量消掉。设$\mathbf{U}(\bm{R})$是随核坐标变化的幺正变换，定义新的电子态与核波函数：
\[\phi_m^{(d)}=\sum_i\phi_i^{(a)}U_{im}, \quad \chi_m^{(d)}=\sum_iU_{im}^*\chi_i^{(a)}\]
这样变换后总波函数$\Psi$本身不变，只是我们换了一套表象来描述同一个物理态。若选择$\mathbf{U}(\bm{R})$满足
\[\nabla_A\mathbf{U}+\mathbf{T}_A^{(1)}\mathbf{U}=0\]
则一阶导数耦合可以被吸收到基底变换中去，核方程化为
\[\left[\hat{T}_{nuc}+\mathbf{V}^{(d)}(\bm{R})-E\mathbbm{1}\right]\bm{\chi}^{(d)}=0, \quad \mathbf{V}^{(d)}(\bm{R})=\mathbf{U}^{\dagger}(\bm{R})\mathbf{V}^{(a)}(\bm{R})\mathbf{U}(\bm{R})\]
此时核动能重新回到了“朴素”的二阶微分形式，但代价是$\mathbf{V}^{(d)}(\bm{R})$一般不再对角，也就是说原本在绝热表象中的动能耦合被转移成了 diabatic 势矩阵的非对角元。对于处理避免交叉、电子跃迁和分子动力学，这种表象往往比绝热表象更方便；但要构造一个全局光滑而且严格的 diabatic 基底通常并不容易，所以实际计算中经常使用局域的 quasi-diabatic 近似。
