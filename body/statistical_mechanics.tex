...
记产生算符$\hat{n}_{\mathbf{k}}$的本征值占据数为$n_{\mathbf{k}}$，此时正则配分函数$Z_N$和为：
...
进而我们可以利用上述展开式重新计算有限温度$T$下理想费米气体的粒子数$N$，能量$E$，压强$P$和热容$C_V$：
\[\begin{aligned}
...
E&=\int_{-\infty}^{\infty} \varepsilon f(\varepsilon)D(\varepsilon)\dd\varepsilon=\frac{2gV}{\pi^2}\left(\frac{m}{2\hbar^2}\right)^{3/2}\left[\int_0^{\mu}\varepsilon^{\frac{3}{2}}\dd{\varepsilon}+\frac{\pi^2}{4}(k_{\rm B}T)^2\mu^{\frac{1}{2}}+\cdots\right]\\
&=\frac{2gV}{\pi^2}\left(\frac{m}{2\hbar^2}\right)^{3/2}\left[\frac{2}{5}\mu^{\frac{5}{2}}+\frac{\pi^2}{4}(k_{\rm B}T)^2\mu^{\frac{1}{2}}+\cdots\right]\\
C_V&=\left(\pdv{E}{T}\right)_{V,N}=gV\left(\frac{m}{2\hbar^2}\right)^{3/2}k_{\rm B}^2T\mu^{\frac{1}{2}}+\cdots\\
P&=\frac{2E}{3V}
=\frac{4g}{3\pi^2}\left(\frac{m}{2\hbar^2}\right)^{3/2}\left[\frac{2}{5}\mu^{\frac{5}{2}}+\frac{\pi^2}{4}(k_{\rm B}T)^2\mu^{\frac{1}{2}}+\cdots\right]\\
\end{aligned}\]
考虑粒子数守恒：
...
考虑相对低温度时$k_{\rm B}T\ll \varepsilon_F$，因此可以近似为：
...
\newpage
除了分布，我们也可以看看一些宏观物理量，如体系内能：
\[\begin{aligned}
U_{\pm}&=-\left(\pdv{\ln\Xi_\pm}{\beta}\right)_{V,\mu}+\mu \expval{N}=-\left[\pdv{}{\beta}\left(\frac{V}{\Lambda^3}f_{5/2}^{\pm}(z)\right)\right]_{V,\mu}+\mu \expval{N}\\
&=-V\left[\pdv{\beta}\left(\Lambda^{-3}\right)f_{5/2}^{\pm}(z)+\Lambda^{-3}\left(\pdv{f_{5/2}^{\pm}(z)}{z}\right)\pdv{z}{\beta}\right]+\mu \expval{N}\\
&=-V\left[-\frac{3}{2\beta\Lambda^3}f_{5/2}^{\pm}(z)+\frac{\mu}{\Lambda^3}f_{3/2}^{\pm}(z)\right]+\mu \expval{N}=\frac{3}{2}\frac{V}{\beta\Lambda^3}f_{5/2}^{\pm}(z)=\frac{3}{2}k_{\rm B}T\frac{V}{\Lambda^3}f_{5/2}^{\pm}(z)=\frac{3}{2}P_\pm V
\end{aligned}\]



理想气体状态方程$PV=Nk_{\rm B}T$。对三维理想量子气体，更方便的做法是引入逸度$z=e^{\beta\mu}$以及特殊函数：
...
这里记号约定为：$-$一律对应费米气体，$+$一律对应玻色气体。于是巨正则配分函数可统一写成：
...
\end{aligned}\]
由巨正则系综中压强$P$和平均粒子数$\expval{N}$与配分函数的关系：
...
在高温低密度极限$z\ll 1$下，因此立刻退化回经典结果$PV=Nk_{\rm B}T$。

若固定粒子数密度$n=N/V$，再引入温度尺度
\[
T_Q=\frac{2\pi\hbar^2}{mk_{\rm B}}n^{2/3}
\]
则有$n\Lambda^3=(T_Q/T)^{3/2}$，从而可把费米气体、玻色气体与经典理想气体的压强放在同一张图中比较：
\begin{figure}[htbp]
\centering
\begin{tikzpicture}
\begin{axis}[
...
\end{axis}
\end{tikzpicture}
\caption{固定数密度$n$下三维理想费米气体、玻色气体与经典理想气体的压强-温度关系。这里横轴取$T/T_Q$，纵轴取$P/(n k_{\rm B}T_Q)$。玻色气体在$T_c/T_Q=\zeta(3/2)^{-2/3}\approx 0.527$处发生 Bose-Einstein 凝聚。}
\end{figure}
