...
更有意思的是，记交换算符为$\hat{P}_{12}$，全同粒子投影算符$\hat{\Pi}_{\pm}=(1\pm \hat{P}_{12})/2$，可以将正则分布算符统一写成$\hat{\rho}_{\rm can}^{(\pm)}=\hat{\Pi}_{\pm}e^{-\beta \hat{H}}/Z_2^{(\pm)}$，我们看密度算符在坐标表象下的矩阵元$\bra{\mathbf{r}_1,\mathbf{r}_2}\hat{\rho}_{\rm can}^{(\pm)}\ket{\mathbf{r}_1',\mathbf{r}_2'}$：
\[\bra{\mathbf{r}_1,\mathbf{r}_2}\hat{\rho}_{\rm can}^{(\pm)}\ket{\mathbf{r}_1',\mathbf{r}_2'}
=\frac{1}{2Z_2^{(\pm)}}\left[\matrixel{\mathbf{r}_1}{e^{-\beta\hat{h}}}{\mathbf{r}_1'}\matrixel{\mathbf{r}_2}{e^{-\beta\hat{h}}}{\mathbf{r}_2'}\pm \matrixel{\mathbf{r}_1}{e^{-\beta\hat{h}}}{\mathbf{r}_2'}\matrixel{\mathbf{r}_2}{e^{-\beta\hat{h}}}{\mathbf{r}_1'}\right]\]
把单粒子热核写成动量积分后，就得到你想要的单粒子积分展开式：
\[\begin{aligned}
\bra{\mathbf{r}_1,\mathbf{r}_2}\hat{\rho}_{\rm can}^{(\pm)}\ket{\mathbf{r}_1',\mathbf{r}_2'}
&=\frac{1}{2Z_2^{(\pm)}(2\pi)^6}\Bigg[
...
我们考虑更多粒子的无相互作用体系，记$\hat{n}_{\mathbf{k}}$为态$\ket{\mathbf{k}}$的产生算符，则哈密顿算符$\hat{H}$与粒子数算符$\hat{N}$：
\[\hat{H}=\sum_{\mathbf{k}} \varepsilon_{\mathbf{k}} \hat{n}_{\mathbf{k}},\quad \hat{N}=\sum_{\mathbf{k}} \hat{n}_{\mathbf{k}}\]
记产生算符$\hat{n}_{\mathbf{k}}$的本征值占据数为$n_{\mathbf{k}}$，此时正则配分函数$Z_N$为：
\[Z_N=\delta\left(\sum_{\mathbf{k}} n_{\mathbf{k}}-N\right)\sum_{\{n_{\mathbf{k}}\}}\exp\left(-\beta\sum_{\mathbf{k}} n_{\mathbf{k}}\varepsilon_{\mathbf{k}}\right)\]
...
上面定义的函数$f(\varepsilon)$就是著名的 Fermi-Dirac 分布函数。
...
\[\expval{n_{\mathbf{k}}}=\left(\sum_{n_{\mathbf{k}}=0}^{\infty}n_{\mathbf{k}}e^{-\beta n_{\mathbf{k}}(\varepsilon_{\mathbf{k}}-\mu)}\right)\Bigg{/}\left(\sum_{n_{\mathbf{k}}=0}^{\infty}e^{-\beta n_{\mathbf{k}}(\varepsilon_{\mathbf{k}}-\mu)}\right)=\frac{e^{-\beta(\varepsilon_{\mathbf{k}}-\mu)}}{1-e^{-\beta(\varepsilon_{\mathbf{k}}-\mu)}}=\frac{1}{e^{\beta(\varepsilon_{\mathbf{k}}-\mu)}-1}\equiv g(\varepsilon_{\mathbf{k}})\]
上面定义的函数$g(\varepsilon)$就是 Bose-Einstein 分布函数。由$\expval{n_{\mathbf{k}}}>0$和函数奇异性要求$\varepsilon_{\mathbf{k}}>\mu$。

在高温极限$\beta(\varepsilon_{\mathbf{k}}-\mu)\gg 1$时，两者都趋于经典 Boltzmann 分布：
\[\expval{n_{\mathbf{k}}} \approx e^{-\beta(\varepsilon_{\mathbf{k}}-\mu)}\]
三者的对比如下：
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{fig/statistical_mechanics/Quantum_and_classical_statistics.png}
\end{figure}
...
以及热容$C_V$和压强$P$，这里引入粒子数密度$n= N/V$简化压强的表达式：
\begin{subequations}
\begin{align}
...
因此化学势不能超过基态能量$\mu\le \varepsilon_0=0$。当固定粒子数密度$n$并降低温度时，$\mu$会从负侧逼近$0$；一旦$\mu$到达$0$，激发态已经无法继续容纳全部粒子，剩余粒子就只能宏观地占据基态，从而发生 Bose-Einstein 凝聚。因此我们需要把基态单独分离出来：
\[N=N_0+N_{\rm ex},\quad N_0=\frac{1}{e^{-\beta\mu}-1}\]
当$\mu\to 0$时，激发态所能容纳的最大粒子数为 [公式\eqref{eq:special_function}]：
...
上面我们计算了量子气体在有限温度下的平均粒子数$\expval{N}$，内能$U$和压强$P$。回到玻色气体的情况，在$T>T_c$时，只剩最关键的化学势$\mu$和热容$C_V$需要处理。先看化学势$\mu$：
\[
n=\frac{N}{V}=\frac{1}{\Lambda^3}f_{3/2}^+(z),\quad z=e^{\beta\mu}.
\]
由公式\eqref{eq:special_function}可知，对玻色气体有
\[
f_{3/2}^+(z)=\sum_{l=1}^{\infty}\frac{z^l}{l^{3/2}}
=z+\frac{z^2}{2^{3/2}}+\frac{z^3}{3^{3/2}}+\cdots,\quad 0<z<1.
\]
因此在给定$n$和$T$时，$z$由下式唯一确定：
\[
f_{3/2}^+(z)=n\Lambda^3.
\]
再利用临界温度处的关系$n\Lambda_c^3=\zeta(3/2)$以及$\Lambda^3=\Lambda_c^3(T_c/T)^{3/2}$，得到
\[
f_{3/2}^+(z)=\zeta\left(\frac{3}{2}\right)\left(\frac{T_c}{T}\right)^{3/2}.
\]
由于$f_{3/2}^+(z)$在$0<z<1$上单调递增，可以把上式反解为
\[
\boxed{\mu(T)=k_{\rm B}T\ln z
=k_{\rm B}T\ln\left\{\left(f_{3/2}^+\right)^{-1}
\left[\zeta\left(\frac{3}{2}\right)\left(\frac{T_c}{T}\right)^{3/2}\right]\right\}<0,\quad T>T_c.}
\]
这里的$\left(f_{3/2}^+\right)^{-1}$表示特殊函数$f_{3/2}^+(z)$关于变量$z$的反函数。若进一步考虑高温低密度极限$z\ll1$，令$y=n\Lambda^3$，则由级数反演得到
\[
z=y-\frac{y^2}{2^{3/2}}+\left(\frac{1}{4}-\frac{1}{3^{3/2}}\right)y^3+\mathcal{O}(y^4).
\]
于是玻色气体化学势的经典极限修正为
\[
\boxed{\frac{\mu}{k_{\rm B}T}
=\ln(n\Lambda^3)-\frac{n\Lambda^3}{2^{3/2}}
+\left(\frac{3}{16}-\frac{1}{3^{3/2}}\right)(n\Lambda^3)^2
+\mathcal{O}\left[(n\Lambda^3)^3\right].}
\]
第一项就是经典理想气体的化学势，后面的负修正来自玻色统计的聚束效应。另一方面，当$T\to T_c^+$时，右端趋于$\zeta(3/2)$，故$z\to1^-$并且$\mu\to0^-$。

为了求热容，需要注意此时$z=z(T)$，不能把它当作常数。先对粒子数守恒式
\[
N=\frac{V}{\Lambda^3}f_{3/2}^+(z)
\]
在$V,N$固定条件下对$T$求导：
\[\begin{aligned}
0&=\left(\pdv{N}{T}\right)_{V,N}
=\frac{V}{\Lambda^3}\left[\frac{3}{2T}f_{3/2}^+(z)+\frac{1}{z}f_{1/2}^+(z)\left(\pdv{z}{T}\right)_{V,N}\right].
\end{aligned}\]
因此
\[
\frac{T}{z}\left(\pdv{z}{T}\right)_{V,N}=-\frac{3}{2}\frac{f_{3/2}^+(z)}{f_{1/2}^+(z)}.
\]
于是热容为
\[\begin{aligned}
C_V&=\left(\pdv{U}{T}\right)_{V,N}
=\frac{3}{2}\frac{Vk_{\rm B}}{\Lambda^3}\left[\frac{5}{2}f_{5/2}^+(z)+\frac{T}{z}f_{3/2}^+(z)\left(\pdv{z}{T}\right)_{V,N}\right]\\
&=\frac{3}{2}\frac{Vk_{\rm B}}{\Lambda^3}\left[\frac{5}{2}f_{5/2}^+(z)-\frac{3}{2}\frac{\left(f_{3/2}^+(z)\right)^2}{f_{1/2}^+(z)}\right]\\
&=\boxed{\frac{3}{2}Nk_{\rm B}\left[\frac{5}{2}\frac{f_{5/2}^+(z)}{f_{3/2}^+(z)}-\frac{3}{2}\frac{f_{3/2}^+(z)}{f_{1/2}^+(z)}\right]}.
\end{aligned}\]
最后再利用理想非相对论气体的关系$P=2U/3V$，得到压强：
\[
\boxed{P=\frac{2U}{3V}=\frac{k_{\rm B}T}{\Lambda^3}f_{5/2}^+(z)}.
\]
当$T\to T_c^+$时有$z\to 1^-$，于是这些结果会平滑连接到临界点附近的玻色气体行为；当$T\gg T_c$时有$z\ll 1$，则会逐渐回到经典理想气体极限。
特别地，在临界点下方有
\[\frac{C_V(T_c^-)}{Nk_{\rm B}}=\frac{15}{4}\frac{\zeta(5/2)}{\zeta(3/2)}\approx 1.93\]
