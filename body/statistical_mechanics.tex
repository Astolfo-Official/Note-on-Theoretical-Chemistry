...
\subsection{微正则系综}\label{sec:wzz}
...
在经典框架内也并不意味着有问题。比如在很多教材中会出现的\textbf{经典定域子系统}，如果考虑经典粒子的话上述定义是完全合理的。但是当我们把统计的边界逐渐推向微观世界时，经典理论的框架就会逐渐失效。Gibbs 和 Boltzmann 等人对气体系统的研究也为后续量子力学的萌芽奠定了基础。当引入量子后即使是定域子系统也需要考虑粒子的全同性。其实把相点的体积取为 Planck 常数也是为了让经典统计力学的结果与量子统计力学的结果在热力学极限下保持一致而引入的一个修正。
...
上述操作是针对单个理想气体分子的，由于计算平均性质，因此把上述结果或者其他性质乘以粒子数$N$，就能得到正则系综下单原子理想气体的热力学性质。这也是 Boltzmann 的做法，但是既然介绍了相空间理论，那我必然不欣赏这种做法，相空间理论提供了一个更统一的框架，还是从（正则）分布函数出发：
\[P_N(E)=\frac{\Omega(N,V,E)e^{-\beta E}}{Z(N,V,T)}\]
对单原子理想气体，在微正则系综中 [章节\ref{sec:wzz}] 我吗已经算出微正则配分函数作为能量$E$的函数为：
\[\Omega(N,V,E)=\frac{V^N}{N!h^{3N}}\frac{(2\pi m)^{3N/2}}{\Gamma(3N/2)}E^{3N/2-1}\]
因此正则配分函数$Z(N,V,T)$为：
\[\begin{aligned}
...
\end{aligned}\]
把这个$Z$代回$P_N(E)$，体积、质量等常数都会约掉，理想气体总动能便服从 Gamma 分布：
\[
...
\[
\expval{E^n}
=\int_0^\infty E^n P_N(E)\dd E
=\frac{\Gamma(3N/2+n)}{\Gamma(3N/2)}\frac{1}{\beta^n}.
\]
取$n=1,2$可得
\[
\expval{E}=\frac{3N}{2\beta}=\frac{3N}{2}k_{\rm B} T,\qquad
\expval{E^2}=\frac{3N}{2}\left(\frac{3N}{2}+1\right)\frac{1}{\beta^2},
\]
于是方差与相对涨落为
\[
\expval{(\Delta E)^2}=\expval{E^2}-\expval{E}^2=\frac{3N}{2}(k_{\rm B} T)^2,\qquad
\frac{\sqrt{\expval{(\Delta E)^2}}}{\expval{E}}=\sqrt{\frac{2}{3N}}.
\]
这最后一个式子很重要：\textcolor{blue}{\textbf{随着粒子数增大，能量的相对涨落按$N^{-1/2}$衰减}}，这就是为什么不同系综在热力学极限下会给出相同的状态方程和热力学关系。

有了$Z$之后，其余热力学量就都可以从正则系综的通式直接求出。先取对数：
\[
\ln Z=-\ln N!+N\ln V-3N\ln\Lambda.
\]
对大体系使用 Stirling 公式$\ln N!\sim N\ln N-N$，便有
\[
\ln Z\sim N\left[\ln\left(\frac{V}{N\Lambda^3}\right)+1\right].
\]
于是
\[
...
\[
U=-\pdv{\ln Z}{\beta}
=-\pdv{}{\beta}\left[\frac{3N}{2}\ln\left(\frac{1}{\beta}\right)\right]
=\frac{3N}{2\beta}
=\frac{3}{2}Nk_{\rm B} T.
\]
熵则为
\[
S=k_{\rm B}\left(\ln Z+\beta U\right)
\sim Nk_{\rm B}\left[\ln\left(\frac{V}{N\Lambda^3}\right)+\frac{5}{2}\right].
\]
化学势可以直接对$F$求偏导：
\[
\mu=\left(\pdv{F}{N}\right)_{V,T}
=-k_{\rm B}T\pdv{}{N}\left\{N\left[\ln\left(\frac{V}{N\Lambda^3}\right)+1\right]\right\}
=k_{\rm B} T\ln(n\Lambda^3).
\]
再由压强定义
\[
P=k_{\rm B} T\left(\pdv{\ln Z}{V}\right)_{N,T}
...
PV=Nk_{\rm B} T.
\]
于是焓、Gibbs 自由能和热容也都能顺次得到：
\[
H=U+PV=\frac{5}{2}Nk_{\rm B} T,\qquad
G=F+PV=\mu N=Nk_{\rm B} T\ln(n\Lambda^3),
\]
\[
C_V=\left(\pdv{U}{T}\right)_{N,V}=\frac{3}{2}Nk_{\rm B},\qquad
