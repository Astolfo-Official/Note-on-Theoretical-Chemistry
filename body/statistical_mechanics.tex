...
\]
根据微正则熵的定义$S_B=k_{\rm B}\ln\Omega_B$，有
\[
\left(\pdv{\ln\Omega_B}{E_B}\right)_{V,N}=\frac{1}{k_{\rm B}}\left(\pdv{S_B}{E_B}\right)_{V,N}=\frac{1}{k_{\rm B} T}.
\]
于是
\[
P_i\propto e^{-\beta E_i},\qquad \beta=\frac{1}{k_{\rm B} T}.
\]
归一化后便得到玻尔兹曼分布：
\[
...
=\frac{1}{Z_N}\exp\left[\frac{S(E,V,N)}{k_{\rm B}}-\beta E\right]
...
\[
\boxed{F(T,V,N)=-k_{\rm B} T\ln Z_N}.
\]
将$F=U-TS$与上式结合，立刻得到熵
\[
\boxed{S=-\left(\pdv{F}{T}\right)_{V,N}=k_{\rm B}\left(\ln Z_N+\beta U\right)}.
\]
对$F$做全微分：
\[
...
所以压强和化学势分别是
\[
\boxed{P=-\left(\pdv{F}{V}\right)_{T,N}=k_{\rm B} T\left(\pdv{\ln Z_N}{V}\right)_{T,N}},
\]
\[
\boxed{\mu=\left(\pdv{F}{N}\right)_{T,V}=-k_{\rm B} T\left(\pdv{\ln Z_N}{N}\right)_{T,V}}.
\]
...
=k_{\rm B}\beta^2\expval{(\Delta E)^2},
\]
也就是
\[
\boxed{\expval{(\Delta E)^2}=k_{\rm B} T^2 C_V}.
\]
...
\]
\[
S=k_{\rm B}\left(\ln Z_N+\beta U\right)
...
\]
\[
U=\frac{3}{2}Nk_{\rm B} T,\qquad P=\frac{Nk_{\rm B} T}{V},
\]
\[
S=Nk_{\rm B}\left[\ln\left(\frac{V}{N\Lambda^3}\right)+\frac{5}{2}\right],
\]
\[
\mu=k_{\rm B} T\ln\left(\frac{N\Lambda^3}{V}\right).
\]
