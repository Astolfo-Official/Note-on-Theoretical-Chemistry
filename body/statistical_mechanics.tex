...
上面定义的函数$g(\varepsilon)$就是 Bose-Einstein 分布函数。
...
\newpage
除了分布，我们也可以看看一些宏观物理量，如理想气体状态方程：
\[
PV=Nk_{\rm B}T
\]
对三维理想量子气体，更方便的做法是引入逸度$z=e^{\beta\mu}$以及特殊函数
\[
f_m^\pm(z)=\frac{1}{\Gamma(m)}\int_0^\infty \frac{x^{m-1}}{z^{-1}e^x\pm 1}\dd x
=\sum_{l=1}^{\infty}\frac{(\mp 1)^{l-1}z^l}{l^m}
\]
其中$f_m^+(z)$对应费米气体，$f_m^-(z)$对应玻色气体。把单粒子态求和替换为动量积分后，可以把巨正则配分函数写成
\[
\ln\Xi_{\pm}=\frac{V}{\Lambda^3}f_{5/2}^{\pm}(z),\qquad
n=\frac{N}{V}=\frac{1}{\Lambda^3}f_{3/2}^{\pm}(z)
\]
于是压强立刻得到
\[
\boxed{P_F=\frac{k_{\rm B}T}{\Lambda^3}f_{5/2}^{+}(z)},\qquad
\boxed{P_B=\frac{k_{\rm B}T}{\Lambda^3}f_{5/2}^{-}(z)}
\]
或者写成更接近经典状态方程的形式：
\[
\boxed{\frac{P_{\pm}V}{Nk_{\rm B}T}=\frac{f_{5/2}^{\pm}(z)}{f_{3/2}^{\pm}(z)}}
\]
在高温低密度极限$z\ll 1$下有$f_m^\pm(z)\approx z$，因此立刻退化回经典结果$PV=Nk_{\rm B}T$。若固定粒子数密度$n=N/V$，再引入温度尺度
\[
