...
\newpage
\subsection{无相互作用量子体系与Fermi-Dirac / Bose-Einstein 分布}
在章节\ref{sec:particle_identity}中我们已经介绍了微观粒子的对称性要求，我们考虑正则系综下温度$T$和体积$V$固定的双无相互作用量子体系，如果这是两个费米子，那么总态必须是反对称的；如果是两个玻色子，则总态必须是对称的。我们在动量表象下分别以$\ket{\mathbf{k}_1}$和$\ket{\mathbf{k}_2}$表示两个单粒子态，$-$表示费米子系统，$+$表示玻色子系统：
\[\ket{\mathbf{k}_1,\mathbf{k}_2}_-=\frac{1}{\sqrt{2}}\left(\ket{\mathbf{k}_1}\otimes\ket{\mathbf{k}_2}-\ket{\mathbf{k}_2}\otimes\ket{\mathbf{k}_1}\right), \quad \ket{\mathbf{k}_1,\mathbf{k}_2}_+=\left\{
\begin{array}{ll}
\left(\ket{\mathbf{k}_1}\otimes\ket{\mathbf{k}_2}+\ket{\mathbf{k}_2}\otimes\ket{\mathbf{k}_1}\right)/\sqrt{2}, & \mathbf{k}_1\neq \mathbf{k}_2\\
\ket{\mathbf{k}}\otimes\ket{\mathbf{k}}, & \mathbf{k}_1=\mathbf{k}_2
\end{array}\right.\]
若记单粒子能谱满足$\hat{h}\ket{\mathbf{k}}=\varepsilon_{\mathbf{k}}\ket{\mathbf{k}}$，且对自由粒子有$\varepsilon_{\mathbf{k}}=\hbar^2k^2/(2m)$，则单粒子正则配分函数为
\[
\begin{aligned}
z_1(\beta)
&=\sum_{\mathbf{k}}e^{-\beta\varepsilon_{\mathbf{k}}}
=\frac{V}{(2\pi)^3}\int \dd[3]{\mathbf{k}}\exp\left(-\frac{\beta\hbar^2k^2}{2m}\right)\\
&=\frac{V}{(2\pi)^3}\cdot 4\pi\int_0^\infty k^2\exp\left(-\frac{\beta\hbar^2k^2}{2m}\right)\dd k
=V\left(\frac{m}{2\pi\beta\hbar^2}\right)^{3/2}
=\frac{V}{\Lambda^3}
\end{aligned}
\]
其中$\Lambda=h/\sqrt{2\pi mk_{\rm B}T}$是热波长。类似地，
\[
z_1(2\beta)=\frac{V}{(2^{3/2}\Lambda^3)}
\]
则两个无相互作用全同粒子的总能量就是$\varepsilon_{\mathbf{k}_1}+\varepsilon_{\mathbf{k}_2}$。对玻色子，允许两个粒子占据同一个单粒子态，因此二粒子正则配分函数为：
\[
\begin{aligned}
Z_2^{(+)}(\beta)
&=\sum_{\mathbf{k}_1\le \mathbf{k}_2}e^{-\beta(\varepsilon_{\mathbf{k}_1}+\varepsilon_{\mathbf{k}_2})}\\
&=\frac{1}{2}\left[\sum_{\mathbf{k}_1,\mathbf{k}_2}e^{-\beta(\varepsilon_{\mathbf{k}_1}+\varepsilon_{\mathbf{k}_2})}+\sum_{\mathbf{k}}e^{-2\beta\varepsilon_{\mathbf{k}}}\right]\\
&=\frac{1}{2}\left[z_1(\beta)^2+z_1(2\beta)\right]
=\frac{1}{2}\left[\left(\frac{V}{\Lambda^3}\right)^2+\frac{V}{2^{3/2}\Lambda^3}\right]
\end{aligned}
\]
对费米子，由于 Pauli 不相容原理，$\mathbf{k}_1=\mathbf{k}_2$的状态被排除，因此：
\[
\begin{aligned}
Z_2^{(-)}(\beta)
&=\sum_{\mathbf{k}_1< \mathbf{k}_2}e^{-\beta(\varepsilon_{\mathbf{k}_1}+\varepsilon_{\mathbf{k}_2})}\\
&=\frac{1}{2}\left[\sum_{\mathbf{k}_1,\mathbf{k}_2}e^{-\beta(\varepsilon_{\mathbf{k}_1}+\varepsilon_{\mathbf{k}_2})}-\sum_{\mathbf{k}}e^{-2\beta\varepsilon_{\mathbf{k}}}\right]\\
&=\frac{1}{2}\left[z_1(\beta)^2-z_1(2\beta)\right]
=\frac{1}{2}\left[\left(\frac{V}{\Lambda^3}\right)^2-\frac{V}{2^{3/2}\Lambda^3}\right]
\end{aligned}
\]
可以看到，\textcolor{blue}{\textbf{玻色子和费米子的区别完全体现在交换项$\pm z_1(2\beta)$上}}：正号意味着玻色子“偏好”占据相同态，负号则体现了费米子的占据排斥。对更多粒子数的情形，直接在对称化/反对称化基底上求和会迅速变得麻烦，因此更自然的做法就是改用占据数表象。
