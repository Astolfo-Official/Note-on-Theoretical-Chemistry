...
\[P_N(E)=\frac{\Omega(N,V,E)e^{-\beta E}}{Z(N,V,T)}, \quad \Omega(N,V,E)=\frac{V^N}{N!h^{3N}}\frac{(2\pi m)^{3N/2}}{\Gamma(3N/2)}E^{3N/2-1}\]
因此正则配分函数$Z(N,V,T)$为：
\[\begin{aligned}
...
\end{aligned}\]
把正则配分函数$Z(N,V,T)$代回正则能量分布$P_N(E)$，发现理想气体总动能便服从$\Gamma$分布：
\[P_N(E)=\frac{\beta^{3N/2}}{\Gamma(3N/2)}E^{3N/2-1}e^{-\beta E} \quad \Rightarrow \quad \expval{E^n}=\int_0^\infty E^n P_N(E)\dd E=\frac{\Gamma(3N/2+n)}{\Gamma(3N/2)}\frac{1}{\beta^n}\]
...
有了正则配分函数$Z(N,V,T)$之后，其余热力学量就都可以从正则系综的通式直接求出：
\[
